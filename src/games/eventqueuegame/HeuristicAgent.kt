...
class HeuristicAgent(val attackRatio: Double, val defenseRatio: Double, val seed: Int = 1) : SimpleActionPlayerInterface {

    val rnd: Random = Random(seed)

    override fun getAgentType(): String = "HeuristicAgent"

    override fun getAction(gameState: ActionAbstractGameState, player: Int): Action {
        if (gameState is LandCombatGame) {
            val attack = attackOption(gameState, player)
            return attack
            val withdraw = withdrawOption(gameState, player)
            return if (withdraw == NoAction) attack else withdraw
        }
        return NoAction
    }
